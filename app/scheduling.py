...
def available_slots(date_str: str) -> list[dict]:
    ...
    while cursor + step <= day_end:
        ...
        if not is_past and not is_locally_booked and not is_calendar_busy:
            # Format time with leading zero removal (works on Windows and POSIX)
            label = cursor.strftime("%I:%M %p").lstrip("0").lstrip(":")
            # If we stripped : (midnight case like ":00"), add it back
