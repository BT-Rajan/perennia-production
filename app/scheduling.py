...
def can_modify(entry: dict) -> bool:
    """True if this appointment can still be rescheduled/cancelled — i.e. it's
    not already cancelled and at least APPT_MIN_NOTICE_HOURS remain before it starts."""
    ...
    return (start - now) >= datetime.timedelta(hours=settings.APPT_MIN_NOTICE_HOURS)
