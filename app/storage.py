...
def _atomic_write_json(path: Path, data: Any) -> None:
    """Write JSON atomically using temporary file + rename. Cleans up temp file on failure."""
    ...
    try:
        ...
        # Clean up temporary file if write/rename fails
        try:
...
def add_appointment(entry: dict[str, Any]) -> dict[str, Any]:
    """Add appointment atomically. Checks availability first to prevent race conditions."""
    with _lock:
        entries = []
        if APPT_PATH.exists():
            try:
        ...
        # Verify the slot is still available (no concurrent booking)
        entry_start = entry.get("start")
        entry_end = entry.get("end")
        for existing in entries:
            if (existing.get("status") != "cancelled" and 
                existing.get("start") == entry_start and 
                existing.get("end") == entry_end):
                raise ValueError(f"Slot {entry_start}-{entry_end} was already booked")
...
def _save_stats(data: dict[str, Any]) -> None:
    """Private: saves stats without locking. Use record_interaction/record_appointment_stat instead."""
...
def record_appointment_stat(date_str: str) -> None:
    """Record an appointment booking. Atomic read-modify-write to prevent lost updates."""
    with _lock:
        data = _load_stats()
