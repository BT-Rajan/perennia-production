...
def build_lead_capture_text(for_lang: str, already_captured: bool) -> str:
    ...
    if for_lang == "ar":
        return (
            ...
            "وجمع هذه البيانات له الأولوية على الإجابة التفصيلية. بأسلوب دافئ ومهني وليس "
            "متكرراً أو مُلحّاً، اجمع ثلاث معلومات بالترتيب التالي، معلومة واحدة في كل رسالة، "
            "منتظراً رد الزائر بينها:\n"
            ...
            "طلبه بشكل صحيح — وليس مجرد إجراء روتيني.\n"
            "إذا طرح الزائر سؤالاً حقيقياً قبل أن تحصل على اسمه، لا تُجب عليه إجابة كاملة "
            "بعد — اكتفِ بجملة قصيرة واحدة تُقر باهتمامه بما يسأل عنه، ثم اطلب اسمه قبل "
            "المتابعة. بمجرد حصولك على اسمه، يمكنك مزج إجابة حقيقية على ما سأله أصلاً أثناء "
            "متابعتك لطلب رقم الهاتف ثم البريد الإلكتروني بالترتيب. لا يحق له إجابة كاملة "
            "ومفصّلة إلا بعد حصولك على المعلومات الثلاث جميعها — لا تتردد حينها في تقديم "
            "إجابة كاملة، ولا تُعد سؤاله عن معلومة حصلت عليها بالفعل. إذا تجاهل الزائر نفس "
            "الطلب بوضوح مرتين متتاليتين، توقف عن السؤال — واصل مساعدته بشكل طبيعي من دون "
            "إعادة الطلب، بدلاً من الاستمرار في منعه إلى ما لا نهاية. لا تنتقل إلى الإجابة "
            "الكاملة عن مواضيع أخرى قبل محاولة جمع هذه المعلومات الثلاث بهذه الطريقة أولاً. "
            "بمجرد حصولك على الاسم ورقم هاتف صالح وبريد إلكتروني صالح، أضِف هذا السطر بالضبط "
            "في نهاية ردّك، مع القيم الفعلية، ولا شيء آخر على ذلك السطر:\n"
    ...
    return (
        ...
        "yet, and collecting them takes priority over answering in depth. In a warm, "
        "professional tone — never repetitive or pushy — collect three things in this exact "
        "order, one per message, waiting for the visitor's reply between each:\n"
        ...
        "follow up properly — frame it as care, not paperwork.\n"
        "If the visitor asks a real question before you have their name, do NOT answer it in "
        "full yet — give at most one short sentence acknowledging what they're asking about, "
        "then ask for their name before going further. Once you have their name, you can weave "
        "in a real answer to what they originally asked while you continue asking for phone, "
        "then email, in turn. Only once you have all three do you owe them the complete, "
        "detailed answer — don't hold back on that once you have all three, and don't keep "
        "re-asking for a detail once you have it. If the visitor clearly brushes off the same "
        "ask twice in a row, drop it — answer their questions normally from then on without "
        "asking again, rather than gating them indefinitely. Don't move on to answering other "
        "topics in full before at least attempting to collect all three this way. Once you have "
        "a name, a valid-looking phone number, and a valid-looking email, append this exact line "
        "at the very end of your reply, with the real values filled in and nothing else on that "
        "line:\n"
        '[[LEAD_CAPTURED name="..." phone="..." email="..."]]\n'
