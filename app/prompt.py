...
def _sanitize_kb_content(text: str, max_lines: int = 50) -> str:
    """Sanitize knowledge base content to prevent prompt injection.
    
    - Limits line count to prevent excessively large injections
    - Adds clear delimiters to isolate injected content
    - Returns sanitized text safe for inclusion in system prompt
    """
    if not text:
        return ""
...
def build_system_prompt(
    ...
    kb_text = ""
    if knowledge_base:
        # Sanitize each document and wrap in clear delimiters to prevent prompt injection
        docs_parts = []
        for f in knowledge_base:
            if f.get("ok"):
                sanitized_text = _sanitize_kb_content(f['text'])
                docs_parts.append(f"--- DOCUMENT START: {f['filename']} ---\n{sanitized_text}\n--- DOCUMENT END ---")
